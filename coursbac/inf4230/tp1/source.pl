...
%%% Donnent le poids pour le dernier chiffre de la série à la position E.
%%% Si le chiffre est 0 (la case vide), le poids est de 0.
%%% Sinon le poids est de 1 .
s_aux(0,0) :- !.
s_aux(_,1).

%%% Donnne le poids pour deux chiffres dans deux cases consécutives.
%%% Si les deux chiffres se suivent le poids est de 0.
%%% Si le chiffre 8 est suivi du chiffre 1, le poids est de 0.
%%% Sinon le poids est de 2 pour toute autre ordre.
%%% Exemple: s_aux(3,4,z) évalue: 	si Y = X + 1 (4 = 3 + 1)
%%% alors poids est 0 et arrête l'évaluation(!).	
