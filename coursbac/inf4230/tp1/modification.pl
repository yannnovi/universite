...
r_fcn(A/B/C/D/E/F/G/H/I,S):- r_aux(A,B,S1), r_aux(B,C,S2), r_aux(C,F,S3),
     r_aux(F,I,S4), r_aux(I,H,S5), r_aux(H,G,S6),
     r_aux(G,D,S7), r_aux(D,A,S8),
     S is S1+S2+S3+S4+S5+S6+S7+S8.

%%% Fonction heuristique Tile Reversal
%%% Cette fonction assigne une pénalité dans le cas ou deux cases ont leur 
%%% position exactement inversée. L'idée est que au moins deux mouvements
%%% supplémentaires sont nécessaires en pareil cas. 
%%% Ex : 2 1 3
%%%      8   4
%%%      7 6 5   
%%%      2 et 1 sont inversées, alors +2 pour la fonction. 
r_aux(X,Y,2):- X is Y+1,!.
r_aux(1,8,2):-!.
r_aux(_,_,0).
