...
% Demande les questions à L'utilisateur et mets les solution possible dans
...
% Utilise le predicat auxiliaire coupe
% Tiré du TP1 de inf2160 automne 2003.
% modification de tri1 pour mettre en ordre décroissant
...
%Affiche le resultat, c'est à dire chaque minéral et sa probabilité.
...
%Base de connaissances pour les minéraux, basée sur la règle de Bayes.
vitreux(diamant,Y) :- Y = 1.  % Y = Probabilité d'avoir une caractéristique étant donné un minéral 
...
vitreux(Z) :- Z = 0.5.		% Z = Probabilité qu'un minéral quelconque ait une de ces caractéristiques
metallique(Z) :- Z = 0.5.
acier(Z) :- Z = 0.3.
non_rayable(Z) :- Z = 0.3.
verre(Z) :- Z = 0.5.

diamant(W) :- W = 0.01.   % W = Probabilité d'avoir un minéral(c'est son abondance)
quartz(W) :- W = 0.3.
pyrite(W) :- W = 0.3.
