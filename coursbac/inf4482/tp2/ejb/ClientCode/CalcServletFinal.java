...
public class CalcServletFinal extends HttpServlet {
	private CalcUnHomeFinal  homecalc1;

	public void init(ServletConfig config) throws ServletException{
		try{
  			// déclaration d'un nouveau contexte
                        InitialContext ctx = new InitialContext();
                        // obtention de l'interface Home du bean qui fera le cal cul
                        Object objref = ctx.lookup("CalcUnBeanFinal");
                        homecalc1 = (CalcUnHomeFinal)PortableRemoteObject.narrow(objref, CalcUnHomeFinal.class);
		} catch (Exception e) {
                }
